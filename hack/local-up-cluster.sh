...
cleanup()
{
  ...
  # Check if the scheduler is still running
  [[ -n "${VMTURBO_PID-}" ]] && VMTURBO_PIDS=$(pgrep -P ${VMTURBO_PID} ; ps -o pid= -p ${VMTURBO_PID})
  [[ -n "${VMTURBO_PIDS-}" ]] && sudo kill ${VMTURBO_PIDS}
...
function start_kubevmturbo {
    VMTURBO_LOG=/tmp/kube-vmturbo.log
    sudo -E "${GO_OUT}/kube-vmturbo" \
      --v=${LOG_LEVEL} \
      --config-path="./plugin/pkg/vmturbo/vmt/metadata/config_nyc.json" \
      --etcd-servers="http://127.0.0.1:4001" \
