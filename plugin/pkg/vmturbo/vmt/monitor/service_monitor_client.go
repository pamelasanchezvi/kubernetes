...
//
func (self *ServiceMonitorClient) TransactionInfo() (transactionInfo []vmtproxy.Transaction, err error) {
	u := self.transactionInfoUrl()
	var ret []vmtproxy.Transaction
	if err = self.httpGetJsonData(&ret, nil, u, "transaction info"); err != nil {
		glog.Errorf("Error getting Json Data for transactions: %s", err)
		return
	}
	transactionInfo = make([]vmtproxy.Transaction, 0, len(ret))
