...
func NewDeployment(meta *vmtmeta.VMTMeta) *Deployment {
	return &Deployment{
		Meta: meta,
	}
}
...
func (deployment *Deployment) GetDestinationFromVmturbo(pod *api.Pod) (map[*api.Pod]string, error) {

	requestSpec := getRequestSpec(pod)

	// reservationResult is map[string]string -- [podName]nodeName
	// TODO !!!!!!! Now only support a single pod.
	reservationResult, err := deployment.RequestPlacement(pod.Name, requestSpec, nil)
	...
	if err != nil {
		glog.Errorf("Error finding deploying destination using vmturbo server: %s", err)
		return nil, err
	}
...
func (deployment *Deployment) RequestPlacement(podName string, requestSpec, filterProperties map[string]string) (map[string]string, error) {
	extCongfix := make(map[string]string)
	extCongfix["Username"] = deployment.Meta.OpsManagerUsername
	extCongfix["Password"] = deployment.Meta.OpsManagerPassword
	vmturboApi := vmtapi.NewVmtApi(deployment.Meta.ServerAddress, extCongfix)
