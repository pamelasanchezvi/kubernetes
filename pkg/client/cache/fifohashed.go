...
// HashedFIFO order processing. If multiple adds/updates of a single item happen while
// an item is in the queue before it has been processed, it will only be
// processed once, and when it is processed, the most recent version will be
// processed. This can't be done with a channel.
//
// HashedFIFO solves this use case:
//  * You want to process every object (exactly) once.
//  * You want to process the most recent version of the object when you process it.
//  * You do not want to process deleted objects, they should be removed from the queue.
//  * You do not want to periodically reprocess objects.
// Compare with DeltaFIFO for other use cases.
type HashedFIFO struct {
	lock sync.RWMutex
	cond sync.Cond
...
func (f *HashedFIFO) Update(obj interface{}) error {
	return f.Add(obj)
}
...
// in the HashedFIFO.
func (f *HashedFIFO) ListKeys() []string {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *HashedFIFO) GetByKey(key string) (item interface{}, exists bool, err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *HashedFIFO) Replace(list []interface{}) error {
	items := map[string]interface{}{}
